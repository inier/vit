...
export default class Service extends EventEmitter {
  ...
  constructor(options: IServiceOptions) {
    ...
    this.pkg = options.pkg || this.resolvePackage();
  ...
  getPluginAPI(options: any) {
    const pluginAPI = new PluginAPI(options);
  ...
  async applyAPI(options: { apply: Function; api: PluginAPI }) {
    let ret = options.apply()(options.api);
    if (isPromise(ret)) {
      ret = await ret;
    }
    return ret || {};
...
${name} from ${plugin.path} register failed.`);
  ...
  async applyPlugins(options: { key: string; type: ApplyPluginsType; initialValue?: any; args?: any }) {
    const hooks = this.hooks[options.key] || [];
    switch (options.type) {
      case ApplyPluginsType.add:
        ...
        for (const hook of hooks) {
          ...
          tAdd.tapPromise(
            ...
            async (memo: any[]) => {
              const items = await hook.fn(options.args);
              return memo.concat(items);
            }
          );
        }
        return await tAdd.promise(options.initialValue || []);
      case ApplyPluginsType.modify:
        ...
        for (const hook of hooks) {
          ...
          tModify.tapPromise(
            ...
            async (memo: any) => {
              return await hook.fn(memo, options.args);
            }
          );
        }
        return await tModify.promise(options.initialValue);
      case ApplyPluginsType.event:
        ...
        for (const hook of hooks) {
          ...
          tEvent.tapPromise(
            ...
            async () => {
              await hook.fn(options.args);
            }
          );
        }
        return await tEvent.promise();
      default:
        throw new Error(`applyPlugin failed, type is not defined or is not matched, got ${options.type}.`);
    }
  }
  ...
  async runCommand({ name, args = {} }: { name: string; args?: any }) {
    ...
    const command =
      typeof this.commands[name] === 'string' ? this.commands[this.commands[name] as string] : this.commands[name];
